...
3. **`pay_next`** — Disburse one installment. **Permissionless** (no auth required) — safety rests on four guards, not on who calls:
   ...
   **Security**: Effects are applied *before* the token transfer (decrement escrow, advance `paid_count`, set `last_paid_ts = now`, potentially set `status = Ended`) — defeating re-entrancy because storage is already updated before the external call. On the final installment, status automatically transitions to `Ended`. Emits `payment` event.
...
5. **`cancel`** — Sender-only. Zeroes escrow and sets `status = Ended` in storage *before* the refund transfer (re-entrancy protection). Conditionally transfers remaining `deposit` back to sender. Emits `cancel` event.

### Security Model

- **Auth**: Every sender-acting call (`init_schedule`, `deposit`, `pause`, `resume`, `cancel`) requires `Address::require_auth` on the *stored* sender — the caller is never trusted to assert who the sender is.
